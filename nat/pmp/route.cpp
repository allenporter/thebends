...
bool GetDefaultGateway(in_addr* addr) {
  ...
  if (nbytes == -1) {
    ...
    warnx("unexpected number of bytes written (%d != %d)", nbytes, len);
    return false;
  }
  bzero(&m, sizeof(struct msg));
  ...
  if (saddr->sin_len != sizeof(struct sockaddr_in)) {
    warnx("invalid default route destination sin_len (%d != %d)",
  ...
  saddr++;
  if (saddr->sin_len != sizeof(struct sockaddr_in)) {
    warnx("invalid default gateway sin_len (%d != %d)", saddr->sin_len,
